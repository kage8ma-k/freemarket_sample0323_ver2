#まずベタ打ちしたDBのカテゴリー・ブランド・サイズ・アイテムのファイルをファイルを空にしてください。
# 14行目以降のコメントアウトを外してrake db:seedをターミナルでうつとデータが入ります。
# カテゴリーのデータを入れないとエラーが出ます。
...
#   # categoryの登録（ここから下のコメントアウトを一度はずす）
